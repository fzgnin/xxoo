...
function get_ordertype($type)
	...
	//制作订单号后四位
	function get_order_id($time,$order_type)
	{
		//查询当前订单编号表里面有没有对应的id
	...
	//制作订单号
	function make_order_sn($num_id,$numstr)
	{
		$time = date("Y-m-d");
		$num = get_order_id($time,$num_id);
		//print_r($num);exit;
		//对num进行处理不足5位自动补零
		$num=sprintf("%05d", $num);
		$order_sn = $numstr.$time.'-'.$num;
		return $order_sn;	
	}
	
	//判断多维数组是否存在空值
	function is_exist_null($array)
	{		
		foreach($array as $v)
		{
			if(is_array($v))
			{
				return is_exist_null($v);
			}else
			{
				if(empty($v))
				{
